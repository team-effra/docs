...
6. For shortfalls that patients need to pay, Effra sends the invoice on the day it is raised. The due date is **5 days by default (D+5)**, and can be configured to **7 days (D+7)**. These use your branding and if the patient replies to ask a question it will go to you, so that you remain in control of the patient experience.
   1. If you're using card on file, Effra sends an extra reminder the day before the due date, then collects the payment automatically on the due date (D+5 by default, or D+7 if configured). If the charge succeeds, we skip the due-date chase email.
